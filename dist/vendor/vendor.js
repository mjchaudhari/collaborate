...
(function() {
  var module = angular.module('ezDirectives');
    this.materialTemplate = [
      '<span>',
        '<img style="height:{{size}}px;width:{{size}}px" ng-class="[rotate, {orange: doRotate}]" src="{{img}}" />',
      '</span>'
    ].join('\n');
  module.directive('cpLogo', [
    '$timeout', function($timeout) {
      return {
        ...
        //controller start
        controller: ["$scope", function ($scope) {      
            $scope.img = angular.isDefined($scope.img) ? $scope.size : './images/cp.png';
            $scope.size = angular.isDefined($scope.size) ? $scope.size : '40';
